...
std::shared_ptr<Model::Assembly> Data::DataStorage::AddAssembly()
{
...
std::shared_ptr<Model::Assembly> Data::DataStorage::AddAssembly( std::string name )
{
...
void Data::DataStorage::SortDataForTiming()
{
  ...
  for( auto& assembly : sortedListOfAssemblies ) {
    ...
    for( auto& [it_part, part] : assembly->GetPartsList() ) {
      auto partString = std::format( "{}{}{}{}{}{}", "\t", part->Name, " ID: ", part->GetID(),
                                     " owned by: ", part->GetOwner()->Name );
    }
  }
}
